    ...
    ?>
        ...
        <tr class="order-remaining">
            ...
            <td data-title="<?php esc_html_e('Remaining','ova-brw'); ?>">
                <strong><?php echo wc_price( $remaining_amount ); ?><?php echo apply_filters( 'ova_remaining_tax_text_html', $tax_text ); ?></strong></td>
        </tr>
    <?php
    }
}
...
function ova_populate_deposit_column($column){
    if ( 'ova_amount_deposit_column' === $column ) {
        ...
        if($order){
            ...
            ?>
                <span class="ova_deposit_amount"><?php echo wc_price( $order_amount_deposit, array( 'currency' => $currency ) ); ?></span>
            <?php
            }
        }
    }
...
function ova_admin_order_item_values( $product , $item , $item_id ){
    ...
    <?php
        ...
        if ( get_option( 'ova_brw_booking_form_show_extra', 'no' ) == 'yes' ) {
            ...
            if ( !empty( $resources ) && is_array( $resources ) ) {
                $resource_html = ovabrw_get_html_resources( $product_id, $resources, $adults_quantity, $childrens_quantity );
            }

            if ( !empty( $services ) && is_array( $services ) ) {
                $service_html = ovabrw_get_html_services($product_id, $services, $adults_quantity, $childrens_quantity );
            }
        ...
        <div class="view">
            <?php
                if ( $ovabrw_amount_insurance ) {
                    echo wc_price( $ovabrw_amount_insurance );
                }
            ?>
        </div>
        <?php if( $product ){ 
    ...
    <td class="ova-deposit-paid" width="1%">
        <div class="view">
            <?php
                if ( $has_deposit ) {
                    echo wc_price( $ovabrw_deposit_amount );
                }
            ?>
        </div>
    ...
    <td class="deposit-remaining" width="1%">
        <div class="view">
            <?php
                if ( $has_deposit ) {
                    echo wc_price( $ovabrw_remaining_amount );
                }
            ?>
        </div>
    ...
    foreach( $order_item as $item ) {
        ...
        if( ! empty( $meta_data ) && is_array( $meta_data ) ) {
            foreach( $meta_data as $data ) {
                if ( $data->key == 'ovabrw_deposit_amount' ) {
...
function ova_order_action_pay_full() {
    ...
    switch ( $action ) {
        ...
        case 'create_remaining_invoice':
            ...
            if ( apply_filters( 'ovabrw_ft_send_email_create_remaining_invoice', true ) ) {
                // Email invoice
                $emails = WC_Emails::instance();
