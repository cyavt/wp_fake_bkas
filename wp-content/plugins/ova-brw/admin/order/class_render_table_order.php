...
class List_Booking extends WP_List_Table {
    ...
    function prepare_items() {
        ...
        $room_query     = $room_id ? 'AND order_item_meta.meta_value = '. sanitize_text_field( $room_id ) : '';
    	        ...
    	        foreach ( $order_line_items as $item_id => $item ) {
    	            ...
    	            foreach ( $item->get_formatted_meta_data() as $meta_id => $meta ) {
                        ...
                        if( $meta->key == 'ovabrw_pickup_date' ){
                            $room_check_in = date( $ovabrw_date_format, strtotime( $meta->value ) );
                        }
                        if( $meta->key == 'ovabrw_pickoff_date' ){
                            $room_check_out = date( $ovabrw_date_format, strtotime( $meta->value ) );
                        }
    	            }
