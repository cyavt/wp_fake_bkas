			...
			<ul>
				<?php foreach ($start_untime as $key => $value) { ?>
					<?php if( $start_untime[$key] != $end_entime[$key] ) : ?>
						<li>
							<?php echo wp_date( $ovabrw_date_time_format, strtotime( $start_untime[$key] ) ).' - '.wp_date( $ovabrw_date_time_format, strtotime( $end_entime[$key] ) ); ?>
						</li>
					<?php else : ?>	
						<li>
							<?php echo wp_date( $ovabrw_date_time_format, strtotime( $start_untime[$key] ) ); ?>
						</li>
					<?php endif; ?>					
				<?php } ?>
			</ul>
