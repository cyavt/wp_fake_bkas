...
<?php if( $ovabrw_label_service ): ?>
	<div class="ovabrw_services rental_item">
		...
		<?php for( $i = 0; $i < count( $ovabrw_label_service ); $i++ ): 
			...
			$ovabrw_service_required 	= isset( $ovabrw_service_required[$i] ) ? $ovabrw_service_required[$i] : '';
			$required = '';

			if ( 'yes' === $ovabrw_service_required ) {
				...
				<select 
					...
					<?php if ( !empty( $ovabrw_service_id ) && isset( $ovabrw_service_id[$i] ) ): ?>
						<?php foreach( $ovabrw_service_id[$i] as $key => $value ): 
							...
							$html_price = sprintf( esc_html__( ' (Adult: %s%s - Children:%s%s)', 'tripgo' ), wc_price($adult_price), $html_duration, wc_price($children_price), $html_duration );
						?>
